...
var clientAddedEvent = function(address, callback) {
  var contract = web3.eth.contract(abi).at(address)
  web3.eth.getBlockNumber((error, latestBlock) => {
    if (error) {
      console.log(error)
    ...
    contract.ClientAdded({}, { fromBlock: latestBlock, toBlock: 'latest' }).watch(function (err, data) {
      ...
      // console.log('Cient adderess from ascii: ' + web3.fromAscii(data.args.cliendAddress))
      var d = {
        clientAddress: data.args.cliendAddress,
        data: web3.toUtf8(data.args.data).replace(/'/g, ''),
        event: 'added'
      }
      callback(err, d)
...
var clientRemovedEvent = function(address, callback) {
  var contract = web3.eth.contract(abi).at(address)
  web3.eth.getBlockNumber((error, latestBlock) => {
    if (error) {
      console.log(error)
...
exports.clientAddedEvent =  Rx.Observable.fromNodeCallback(clientAddedEvent)
exports.clientRemovedEvent =  Rx.Observable.fromNodeCallback(clientRemovedEvent)